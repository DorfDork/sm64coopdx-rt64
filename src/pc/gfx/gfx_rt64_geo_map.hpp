...
#define FILL_GEO_MAPS(x) do { geoNameMap[(void *)(x)] = #x; nameGeoMap[#x] = (void *)(x); } while (0)
...
inline void gfx_rt64_init_geo_layout_maps(std::unordered_map<void *, std::string> &geoNameMap, std::map<std::string, void *> &nameGeoMap) {
    ...
    FILL_GEO_MAPS(intro_geo_0002D0);
    FILL_GEO_MAPS(intro_geo_mario_head_regular);
    FILL_GEO_MAPS(intro_geo_mario_head_dizzy);
    FILL_GEO_MAPS(intro_geo_000414);
    FILL_GEO_MAPS(breakable_box_small_geo);

    // Extra playable characters
    FILL_GEO_MAPS(error_model_geo);
    FILL_GEO_MAPS(luigi_geo);
    FILL_GEO_MAPS(luigis_cap_geo);
    FILL_GEO_MAPS(luigis_metal_cap_geo);
    FILL_GEO_MAPS(luigis_wing_cap_geo);
    FILL_GEO_MAPS(luigis_winged_metal_cap_geo);
    FILL_GEO_MAPS(wario_geo);
    FILL_GEO_MAPS(warios_cap_geo);
    FILL_GEO_MAPS(warios_metal_cap_geo);
    FILL_GEO_MAPS(warios_wing_cap_geo);
    FILL_GEO_MAPS(warios_winged_metal_cap_geo);
    FILL_GEO_MAPS(waluigi_geo);
    FILL_GEO_MAPS(waluigis_cap_geo);
    FILL_GEO_MAPS(waluigis_metal_cap_geo);
    FILL_GEO_MAPS(waluigis_wing_cap_geo);
    FILL_GEO_MAPS(waluigis_winged_metal_cap_geo);
    FILL_GEO_MAPS(toad_player_geo);
    FILL_GEO_MAPS(toads_cap_geo);
    FILL_GEO_MAPS(toads_metal_cap_geo);
    FILL_GEO_MAPS(toads_wing_cap_geo);
    FILL_GEO_MAPS(toads_winged_metal_cap_geo);
}
